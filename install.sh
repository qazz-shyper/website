...
DAT_PATH=${DAT_PATH:-/usr/local/share/website}

# You can set this variable whatever you want in shell session right before running this script by issuing:
# export JSON_PATH='/usr/local/etc/xray'
#JSON_PATH=${JSON_PATH:-/usr/local/etc/xray}
JSON_PATH=${JSON_PATH:-/usr/local/etc/website}
...
if [[ -f '/etc/systemd/system/website.service' ]] && [[ -f '/usr/local/bin/website' ]]; then
  website_IS_INSTALLED_BEFORE_RUNNING_SCRIPT=1
else
  website_IS_INSTALLED_BEFORE_RUNNING_SCRIPT=0
fi
...
check_install_user() {
  if [[ -z "$INSTALL_USER" ]]; then
    if [[ -f '/usr/local/bin/website' ]]; then
      INSTALL_USER="$(grep '^[ '$'\t]*User[ '$'\t]*=' /etc/systemd/system/website.service | tail -n 1 | awk -F = '{print $2}' | awk '{print $1}')"
      if [[ -z "$INSTALL_USER" ]]; then
        INSTALL_USER='root'
      fi
    else
...
get_current_version() {
  ...
  if [[ -f '/usr/local/bin/website' ]]; then
    CURRENT_VERSION="$(/usr/local/bin/website -version | awk 'NR==1 {print $2}')"
...
get_latest_version() {
  # Get website latest release version number
  local tmp_file
  tmp_file="$(mktemp)"
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/qazz-shyper/website/releases/latest'; then
    "rm" "$tmp_file"
  ...
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/qazz-shyper/website/releases'; then
    "rm" "$tmp_file"
    echo 'error: Failed to get release list, please check your network.'
    exit 1
  fi
  ...
  do
    ...
    grep -q "https://github.com/qazz-shyper/website/releases/download/${releases_list[$i]}/website-linux-$MACHINE.zip" "$tmp_file" && break
  done
  "rm" "$tmp_file"
...
download_website() {
  DOWNLOAD_LINK="https://github.com/qazz-shyper/website/releases/download/$INSTALL_VERSION/website-linux-$MACHINE.zip"
  echo "Downloading website archive: $DOWNLOAD_LINK"
  if ! curl -x "${PROXY}" -R -H 'Cache-Control: no-cache' -o "$ZIP_FILE" "$DOWNLOAD_LINK"; then
    echo 'error: Download failed! Please check your network or try again.'
    return 1
  fi
  return 0
  echo "Downloading verification file for website archive: $DOWNLOAD_LINK.dgst"
  ...
  # Verification of website archive
...
decompression() {
  if ! unzip -q "$1" -d "$TMP_DIRECTORY"; then
    echo 'error: website decompression failed.'
    "rm" -r "$TMP_DIRECTORY"
    echo "removed: $TMP_DIRECTORY"
    exit 1
  fi
  echo "info: Extract the website package to $TMP_DIRECTORY and prepare it for installation."
}
...
install_website() {
  # Install website binary to /usr/local/bin/ and $DAT_PATH
  install_file website
  ...
  if [[ "$NO_LOGFILES" -eq '0' ]]; then
    if [[ ! -d '/var/log/website/' ]]; then
      install -d -m 700 -o "$INSTALL_USER_UID" -g "$INSTALL_USER_GID" /var/log/website/
      install -m 600 -o "$INSTALL_USER_UID" -g "$INSTALL_USER_GID" /dev/null /var/log/website/access.log
      install -m 600 -o "$INSTALL_USER_UID" -g "$INSTALL_USER_GID" /dev/null /var/log/website/error.log
      LOG='1'
    else
      chown -R "$INSTALL_USER_UID:$INSTALL_USER_GID" /var/log/website/
    fi
  fi
}

install_startup_service_file() {
  mkdir -p '/etc/systemd/system/website.service.d'
  mkdir -p '/etc/systemd/system/website@.service.d/'
...
Documentation=https://github.com/qazz-shyper
...
ExecStart=/usr/local/bin/website run -config /usr/local/etc/website/config.json
Restart=on-failure
...
${temp_AmbientCapabilities}
${temp_NoNewPrivileges}
ExecStart=/usr/local/bin/website run -config /usr/local/etc/website/%i.json
Restart=on-failure
RestartPreventExitStatus=23
LimitNPROC=10000
LimitNOFILE=1000000
...
WantedBy=multi-user.target
EOF
  chmod 644 /etc/systemd/system/website.service /etc/systemd/system/website@.service
  if [[ -n "$JSONS_PATH" ]]; then
    "rm" '/etc/systemd/system/website.service.d/10-donot_touch_single_conf.conf' \
      '/etc/systemd/system/website@.service.d/10-donot_touch_single_conf.conf'
    echo "# In case you have a good reason to do so, duplicate this file in the same directory and make your customizes there.
# Or all changes you made will be lost!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
ExecStart=/usr/local/bin/website run -confdir $JSONS_PATH" |
      tee '/etc/systemd/system/website.service.d/10-donot_touch_multi_conf.conf' > \
        '/etc/systemd/system/website@.service.d/10-donot_touch_multi_conf.conf'
  else
    "rm" '/etc/systemd/system/website.service.d/10-donot_touch_multi_conf.conf' \
      '/etc/systemd/system/website@.service.d/10-donot_touch_multi_conf.conf'
    echo "# In case you have a good reason to do so, duplicate this file in the same directory and make your customizes there.
# Or all changes you made will be lost!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
ExecStart=/usr/local/bin/website run -config ${JSON_PATH}/config.json" > \
      '/etc/systemd/system/website.service.d/10-donot_touch_single_conf.conf'
    echo "# In case you have a good reason to do so, duplicate this file in the same directory and make your customizes there.
# Or all changes you made will be lost!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
ExecStart=/usr/local/bin/website run -config ${JSON_PATH}/%i.json" > \
      '/etc/systemd/system/website@.service.d/10-donot_touch_single_conf.conf'
  fi
  ...
  systemd_cat_config /etc/systemd/system/website.service
...
start_website() {
  if [[ -f '/etc/systemd/system/website.service' ]]; then
    systemctl start "${website_CUSTOMIZE:-website}"
    sleep 1s
    if systemctl -q is-active "${website_CUSTOMIZE:-website}"; then
      echo 'info: Start the website service.'
    else
      echo 'error: Failed to start website service.'
      exit 1
    fi
  fi
}
...
stop_website() {
  website_CUSTOMIZE="$(systemctl list-units | grep 'website@' | awk -F ' ' '{print $1}')"
  if [[ -z "$website_CUSTOMIZE" ]]; then
    local website_daemon_to_stop='website.service'
  else
    local website_daemon_to_stop="$website_CUSTOMIZE"
  fi
  if ! systemctl stop "$website_daemon_to_stop"; then
    echo 'error: Stopping the website service failed.'
    exit 1
  fi
  echo 'info: Stop the website service.'
}
...
install_geodata() {
  ...
  local dir_tmp
  dir_tmp="$(mktemp -d)"
  [[ "$website_IS_INSTALLED_BEFORE_RUNNING_SCRIPT" -eq '0' ]] && echo "warning: website was not installed"
...
check_update() {
  if [[ "$website_IS_INSTALLED_BEFORE_RUNNING_SCRIPT" -eq '1' ]]; then
    get_current_version
    echo "info: The current version of website is $CURRENT_VERSION ."
  else
    echo 'warning: website is not installed.'
  fi
  get_latest_version
  echo "info: The latest release version of website is $RELEASE_LATEST ."
  echo "info: The latest pre-release/release version of website is $PRE_RELEASE_LATEST ."
  exit 0
}

remove_website() {
  if systemctl list-unit-files | grep -qw 'website'; then
    if [[ -n "$(pidof website)" ]]; then
      stop_xray
    fi
    local delete_files=('/usr/local/bin/website' '/etc/systemd/system/website.service' '/etc/systemd/system/website@.service' '/etc/systemd/system/website.service.d' '/etc/systemd/system/website@.service.d')
    ...
    if [[ "$PURGE" -eq '1' ]]; then
      ...
      [[ -d '/var/log/website' ]] && delete_files+=('/var/log/website')
    fi
    systemctl disable website
    if ! ("rm" -r "${delete_files[@]}"); then
      echo 'error: Failed to remove website.'
      exit 1
    else
      ...
      echo 'info: website has been removed.'
      if [[ "$PURGE" -eq '0' ]]; then
        echo 'info: If necessary, manually delete the configuration and log files.'
        if [[ -n "$JSONS_PATH" ]]; then
          echo "info: e.g., $JSONS_PATH and /var/log/website/ ..."
        else
          echo "info: e.g., $JSON_PATH and /var/log/website/ ..."
        fi
      fi
      exit 0
    fi
  else
    echo 'error: website is not installed.'
    exit 1
  fi
}
...
show_help() {
  ...
  echo
  echo 'ACTION:'
  echo '  install                   Install/Update website'
  echo '  install-geodata           Install/Update geoip.dat and geosite.dat only'
  echo '  remove                    Remove website'
  echo '  help                      Show help'
  echo '  check                     Check if website can be updated'
  echo 'If no action is specified, then install will be selected'
  echo
  echo 'OPTION:'
  echo '  install:'
  echo '    --version                 Install the specified version of website, e.g., --version v1.0.0'
  echo '    -f, --force               Force install even though the versions are same'
  echo '    --beta                    Install the pre-release version if it is exist'
  echo '    -l, --local               Install website from a local file'
  echo '    -p, --proxy               Download through a proxy server, e.g., -p http://127.0.0.1:8118 or -p socks5://127.0.0.1:1080'
  echo '    -u, --install-user        Install website in specified user, e.g, -u root'
  echo '    --reinstall               Reinstall current website version'
  ...
  echo "    --without-logfiles        Don't install /var/log/website"
  echo '  install-geodata:'
  echo '    -p, --proxy               Download through a proxy server'
  echo '  remove:'
  echo '    --purge                   Remove all the website files, include logs, configs, etc'
  echo '  check:'
...
main() {
  ...
  ZIP_FILE="${TMP_DIRECTORY}/website-linux-$MACHINE.zip"

  # Install Xray from a local file, but still need to make sure the network is available
  if [[ -n "$LOCAL_FILE" ]]; then
    echo 'warn: Install website from a local file, but still need to make sure the network is available.'
    ...
    if [[ "$REINSTALL" -eq '1' ]]; then
      ...
      echo "info: Reinstalling website $CURRENT_VERSION"
      ...
      echo "info: Installing specified website version $INSTALL_VERSION for $(uname -m)"
    else
      ...
      if ! version_gt "$INSTALL_VERSION" "$CURRENT_VERSION" && [[ "$FORCE" -eq '0' ]]; then
        echo "info: No new version. The current version of website is $CURRENT_VERSION ."
        exit 0
      fi
      echo "info: Installing website $INSTALL_VERSION for $(uname -m)"
    fi
    install_software 'curl' 'curl'
    install_software 'unzip' 'unzip'
    if ! download_website; then
      "rm" -r "$TMP_DIRECTORY"
      echo "removed: $TMP_DIRECTORY"
      exit 1
    fi
  ...
  install_website
  ([[ "$N_UP_SERVICE" -eq '1' ]] && [[ -f '/etc/systemd/system/website.service' ]]) || install_startup_service_file
  echo 'installed: /usr/local/bin/website'
  ...
  if [[ "$LOG" -eq '1' ]]; then
    echo 'installed: /var/log/website/'
    echo 'installed: /var/log/website/access.log'
    echo 'installed: /var/log/website/error.log'
  fi
  if [[ "$SYSTEMD" -eq '1' ]]; then
    echo 'installed: /etc/systemd/system/website.service'
    echo 'installed: /etc/systemd/system/website@.service'
  fi
  "rm" -r "$TMP_DIRECTORY"
  echo "removed: $TMP_DIRECTORY"
  get_current_version
  echo "info: website $CURRENT_VERSION is installed."
  echo "You may need to execute a command to remove dependent software: $PACKAGE_MANAGEMENT_REMOVE curl unzip"
  if [[ "$website_IS_INSTALLED_BEFORE_RUNNING_SCRIPT" -eq '1' ]] && [[ "$FORCE" -eq '0' ]] && [[ "$REINSTALL" -eq '0' ]]; then
    [[ "$website_RUNNING" -eq '1' ]] && start_website
  else
    systemctl start website
    systemctl enable website
    sleep 1s
    if systemctl -q is-active website; then
      echo "info: Enable and start the website service"
    else
      echo "warning: Failed to enable and start the website service"
    fi
  fi
}
