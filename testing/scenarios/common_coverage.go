...
func BuildWebsite() error {
	...
	cmd := exec.Command("go", "test", "-tags", "coverage coveragemain", "-coverpkg", "github.com/qazz-shyper/website/...", "-c", "-o", testBinaryPath, GetSourcePath())
	return cmd.Run()
}

func RunWebsiteProtobuf(config []byte) *exec.Cmd {
	genTestBinaryPath()
